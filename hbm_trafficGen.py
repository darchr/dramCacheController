...
# build/NULL/gem5.opt traffGen.py hbm_2000 1GB 32 linear 10000000 128MiB 10000 100 nvm_2400
...
system.mem_ctrl.partitioned_q = False

#memr = AddrRange(start = '0', end = '256MB', masks = [1 << 6], intlvMatch = 0)
#memr2 = AddrRange(start = '0', end = '256MB', masks = [1 << 6], intlvMatch = )

system.mem_ctrl.dram = MemTypes[options.dram](range=AddrRange(start = '0', end = '512MB', masks = [1 << 6], intlvMatch = 0))
system.mem_ctrl.dram_2 = MemTypes[options.dram](range=AddrRange(start = '0', end = '512MB', masks = [1 << 6], intlvMatch = 1))

system.mem_ctrl.min_reads_per_switch = 64
system.mem_ctrl.min_writes_per_switch = 64

#system.mem_ctrl.mem_2 = MemTypes[options.dram](range=AddrRange('0', '256MB', [1 << 6], 0))
#system.mem_ctrl.mem_2 = MemTypes[options.dram](range=AddrRange(Addr('128MB'), size = '128MB'))

#system.mem_ctrl.nvm = MemTypes[options.nvm](range=AddrRange('8GB'))

#system.mem_ctrl.dram.tREFI = "2000"
#system.mem_ctrl.dram_2.tREFI = "2000"
...
def createRandomTraffic(tgen):
    yield tgen.createRandom(options.duration,   # duration
                            0,              # min_addr
                            AddrRange('512MB').end,#AddrRange(options.max_address).end,  # max_adr
                            64,             # block_size
