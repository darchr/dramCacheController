...
from m5.objects.DCMemInterface import *
...
class DRAMDCInterface(DCMemInterface):
    type = 'DRAMDCInterface'
...
class DDR4_2400_16x4(DRAMDCInterface):
    # size of device
    device_size = '1GiB'
...
class LPDDR2_S4_1066_1x32(DRAMDCInterface):
    # No DLL in LPDDR2
    dll = False
...
class WideIO_200_1x128(DRAMDCInterface):
    # No DLL for WideIO
    dll = False
...
class LPDDR3_1600_1x32(DRAMDCInterface):
    # No DLL for LPDDR3
    dll = False
...
# A single DDR5-8400 x64 channel (one command and address bus), with
# timings based on a DDR5-8400 (DDR5-8400AN)
# in an 16x4 configuration.
# CL-nRCD-nRP --> 46-46-46?
# page 400 of the data sheet
class DDR5_8400_2x8(DRAMDCInterface):
    # size of device
    device_size = '4GiB'
    ...
    # @8400 data rate, tCCD_L is 6 CK ?
    # CAS-to-CAS delay for bursts to the same bank group
    # tBURST is equivalent to tCCD_S; no explicit parameter required
    # for CAS-to-CAS delay for bursts to different bank groups

    # For 8400 MT/s, the number is max(8nCK, 5ns)
    tCCD_L = '5ns';
    ...
    # Default same rank rd-to-wr bus turnaround to 2 CK, @4200 MHz = 0.476 ns
    tRTW = '0.476ns'

    # Default different rank bus delay to 2 CK, @4200 MHz = 0.476 ns
    tCS = '0.476ns' #did not find
...
class GDDR5_4000_2x32(DRAMDCInterface):
    # size of device
    device_size = '128MiB'
...
class HBM_1000_4H_1x128(DRAMDCInterface):
...
class LPDDR5_5500_1x16_BG_BL32(DRAMDCInterface):
